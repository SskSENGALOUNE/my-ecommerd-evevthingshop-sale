...
import { CustomerGoogleLoginCommand } from "../../application/auth/commands/customer-google-login.command";
...
export class AuthController {
  ...
  async customerLogin(@Body() dto: CustomerLoginDto, @Res() res: Response) {
    const result = await this.commandBus.execute(
      new CustomerLoginCommand(dto.email, dto.password),
    );
  ...
  async customerRegister(
    ...
    const result = await this.commandBus.execute(
      new CustomerRegisterCommand(dto.email, dto.password, dto.name, dto.phone),
    );
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: "Customer login via Google (Supabase OAuth)" })
  async customerGoogleLogin(
    @Body() dto: CustomerGoogleLoginDto,
    @Res() res: Response,
  ) {
    const result = await this.commandBus.execute(
      new CustomerGoogleLoginCommand(dto.accessToken),
    );
